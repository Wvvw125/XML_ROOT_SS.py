...
class setset(object):
 def __init__(self, path,f_name1):
    ...
    #   Run.print_cfg().O()      # 白边配置模式归零
  #  Run.head_cfg().Oset_HeadOne_0()  # 所有喷头设置 为1号头数据
  #  Run.head_cfg().Oset_NunHead400_O(2)  #归零 第几个喷头参数
    # Run.head_cfg().Oset_NunHead400_O(4)  #归零 第几个喷头参数
    #  g().SetuseHeadnum(4)      # 设置实际使用喷头数量
...
if __name__ == '__main__':
 path= "F:\售后体系库\……【客户软件库】……\V2.0平台\国内\恒恒泰_无logo需求\BetterPrinterApp_PB_7610\SoftConfig"  #'E:\2.0打印软件2022年1月\G5I\SoftConfig'  # 目标文件夹
